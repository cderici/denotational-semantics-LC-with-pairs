...
lemma e_cons_intro[intro]: "\<lbrakk> v1 \<in> E e1 \<rho>; v2 \<in> E e2 \<rho>; v = VPair v1 v2 \<rbrakk> 
  \<Longrightarrow> v \<in> E (ECons e1 e2) \<rho>" by auto

lemma e_cons_elim[elim]: "\<lbrakk> v \<in> E (ECons e1 e2) \<rho>;
  \<And> v1 v2. \<lbrakk> v1 \<in> E e1 \<rho>; v2 \<in> E e2 \<rho>; v \<sqsubseteq> VPair v1 v2 \<rbrakk> \<Longrightarrow> P \<rbrakk> \<Longrightarrow> P" by auto

lemma e_car_intro[intro]: "\<lbrakk> (VPair v1 v2) \<in> E e \<rho>; v = v1\<rbrakk> \<Longrightarrow> v \<in> E (ECar e) \<rho>"
  using DeclSemAsDenotFSet.Ecar by blast

lemma e_car_elim[elim]: "\<lbrakk> v \<in> E (ECar e) \<rho>; 
  \<And> v1 v2. \<lbrakk> VPair v1 v2 \<in> E e \<rho>; v \<sqsubseteq> v1\<rbrakk> \<Longrightarrow> P \<rbrakk> \<Longrightarrow> P" using val_le_refl by auto

lemma e_cdr_intro[intro]: "\<lbrakk> VPair v1 v2 \<in> E e \<rho>; v = v2 \<rbrakk> \<Longrightarrow> v \<in> E (ECdr e) \<rho>"
  by auto

lemma e_cdr_elim[elim]: "\<lbrakk> v \<in> E (ECdr e) \<rho>;
  \<And> v1 v2. \<lbrakk> VPair v1 v2 \<in> E e \<rho>; v \<sqsubseteq> v2 \<rbrakk> \<Longrightarrow> P \<rbrakk> \<Longrightarrow> P" by auto
...
lemma e_app_intro[intro]: "\<lbrakk> VFun f \<in> E e1 \<rho>; v2 \<in> E e2 \<rho>; (v2',v3') \<in> fset f; v2' \<sqsubseteq> v2; v \<sqsubseteq> v3'\<rbrakk>
     \<Longrightarrow> v \<in> E (EApp e1 e2) \<rho>" by auto 
...
next
  case (ECons e1 e2)
  from ECons(3) obtain v1 v2 where v: "v = VPair v1 v2" and 
    e1_v1: "v1 \<in> E e1 \<rho>" and e2_v2: "v2 \<in> E e2 \<rho>" by auto
  from v ECons(5) obtain v1' v2' where vp: "v' = VPair v1' v2'"
    by (case_tac v') auto

  from ECons(4) have fv_e1: "FV e1 \<turnstile> \<rho> \<sqsubseteq> \<rho>'" and
    fv_e2: "FV e2 \<turnstile> \<rho> \<sqsubseteq> \<rho>'" by auto
    
  from ECons(1) e1_v1 fv_e1 have v1_envp: "v1 \<in> E e1 \<rho>'" by auto
  from ECons(2) e2_v2 fv_e2 have v2_envp: "v2 \<in> E e2 \<rho>'" by auto

  from ECons(5) v vp e1_v1 have v1_v1: "v1' \<sqsubseteq> v1" by auto
  from ECons(5) v vp e2_v2 have v2_v2: "v2' \<sqsubseteq> v2" by auto

  from ECons(1) v1_v1 v1_envp have 1:  "v1' \<in> E e1 \<rho>'" by auto
  from ECons(2) v2_v2 v2_envp have 2:  "v2' \<in> E e2 \<rho>'" by auto

  from vp 1 2 show ?case by auto
next
  case (ECar e)
  from ECar(2) obtain v1 v2 where p_e: "VPair v1 v2 \<in> E e \<rho>" and v: "v \<sqsubseteq> v1" by blast
  from ECar(3) have fv_e: "FV e \<turnstile> \<rho> \<sqsubseteq> \<rho>'" by auto
  from ECar(1) fv_e p_e have p_e_rp: "VPair v1 v2 \<in> E e \<rho>'" by auto
  from ECar(4) v have vp: "v' \<sqsubseteq> v1" by (rule val_le_trans)
  from p_e_rp vp show ?case by (meson ECar.IH e_car_intro fv_e p_e val_le_refl vpair_le)
next
  case (ECdr e)
  from ECdr(2) obtain v1 v2 where p_e: "VPair v1 v2 \<in> E e \<rho>" and v: "v \<sqsubseteq> v2" by blast
  from ECdr(3) have fv_e: "FV e \<turnstile> \<rho> \<sqsubseteq> \<rho>'" by auto
  from ECdr(1) fv_e p_e have p_e_rp: "VPair v1 v2 \<in> E e \<rho>'" by auto
  from ECdr(4) v have vp: "v' \<sqsubseteq> v2" by (rule val_le_trans)
  from p_e_rp vp show ?case by (meson ECdr.IH e_cdr_intro fv_e p_e val_le_refl vpair_le)
next
  case (EApp e1 e2)
