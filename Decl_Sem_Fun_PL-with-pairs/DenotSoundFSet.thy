...
lemma subst_cons: "subst x v (ECons e1 e2) = ECons (subst x v e1) (subst x v e2)" 
  by auto

lemma subst_car: "subst x v (ECar e) = ECar (subst x v e)" 
  by auto

lemma subst_cdr: "subst x v (ECdr e) = ECdr (subst x v e)"
  by auto
...
next
  case (7 e1 e2 \<rho>)
  then show ?case
    apply (simp only: subst_cons) apply (erule e_cons_elim)
    apply simp apply (metis e_sub le_any_pair_inv) done
next
  case (8 e \<rho>)
  then show ?case
    apply (simp only: subst_car) apply fastforce done
next
  case (9 e \<rho>)
  then show ?case
    apply (simp only: subst_cdr) apply fastforce done
qed
...
proof (induction arbitrary: v \<rho> rule: reduce.induct)
 ...
 next
   ...
   then show ?case by auto
 next
   case (car_v v1 v2 v \<rho>)
   then show ?case by (metis e_car_elim e_cons_elim e_sub vpair_le_inv)
 next
   case (cdr_v v1 v2 v \<rho>)
   then show ?case by (metis e_cdr_elim e_cons_elim e_sub vpair_le_inv)
qed force+
...
lemma canonical_pair: assumes v: "VPair v1 v2 \<in> E v \<rho>" and vv: "isval v" shows 
"\<exists> v1' v2'. v = ECons v1' v2'" using v vv by (cases v) auto
...
next
  case (7 e1 e2 \<rho>)
  show ?case
    apply (rule e_cons_elim) 
     apply(cases "is_val e1")
      apply(cases "is_val e2") apply (simp add: valpair) apply force apply force
     apply blast
    using 7 apply simp
    apply (meson cons_left cons_right valpair) done
next
  case (8 e \<rho>)
  show ?case
    apply (rule e_car_elim) using 8(2) apply assumption
    apply (cases "is_val e") apply (frule canonical_pair) apply force apply (erule exE)+
     apply simp apply (rule disjI2) apply (meson car_v is_val_def isval_cons_inv)
    apply (metis "8.IH" "8.prems"(2) "8.prems"(3) FV.simps(8) car) done
next
  case (9 e \<rho>)
  show ?case
    apply (rule e_cdr_elim) using 9(2) apply assumption
    apply (cases "is_val e") apply (frule canonical_pair) apply force apply (erule exE)+
     apply simp apply (rule disjI2) apply (meson cdr_v is_val_def isval_cons_inv)
    apply (metis "9.IH" "9.prems"(2) "9.prems"(3) FV.simps(9) cdr) done
qed auto
...
primrec good :: "val \<Rightarrow> bval set" where
  ...
  Gpair: "good (VPair v1 v2) = { vp. \<exists> bv1 bv2. vp = BPair bv1 bv2 
          \<and> bv1 \<in> (good v1) \<and> bv2 \<in> (good v2)}"
...
lemma good_pair: "w \<in> good (VPair v1 v2) \<Longrightarrow> \<exists> b1 b2. w = BPair b1 b2 \<and> b1 \<in> good v1 \<and> b2 \<in> good v2"
  by simp

lemma sub_good: fixes v::val assumes wv: "w \<in> good v" and vp_v: "v' \<sqsubseteq> v" shows "w \<in> good v'"
  using wv vp_v
proof (induction v arbitrary: v' w)
  case (VNat n)
...
next
  ...
  qed
  from gt2 b w show ?case by (simp add: good_fun_def)
next
  case (VPair v1 v2)
  from VPair obtain v1' v2' where vp: "v' = VPair v1' v2'" and 
    v1_v1: "v1' \<sqsubseteq> v1" and v2_v2: "v2' \<sqsubseteq> v2" by blast

  from VPair(3) good_pair obtain b1 b2 where w: "w = BPair b1 b2" by blast

  from VPair Gpair w have bv1_v1: "b1 \<in> good v1" and bv2_v2: "b2 \<in> good v2" 
     apply blast using VPair.prems(1) w by auto

  from w bv1_v1 v1_v1 VPair(1) have 1: "b1 \<in> good v1'" by blast
  from w bv2_v2 v2_v2 VPair(2) have 2: "b2 \<in> good v2'" by blast

  from 1 2 show ?case by (simp add: vp w)
qed
...
next -- "EIf"
  ...
  qed
next -- "ECons"
  case (7 e1 e2 \<rho> v' \<rho>')
  show ?case using "7.IH"(1) "7.IH"(2) "7.prems"(1) "7.prems"(2) by fastforce
next -- "ECar"
  case (8 e \<rho> v' \<rho>')
  then show ?case by  (smt e_car_elim eval_car good.simps(3) mem_Collect_eq sub_good)
next -- "ECdr"
  case (9 e \<rho> v' \<rho>')
  then show ?case by (smt e_cdr_elim eval_cdr good.simps(3) mem_Collect_eq sub_good)
qed
