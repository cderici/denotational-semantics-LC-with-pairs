...
inductive eval :: "benv \<Rightarrow> exp \<Rightarrow> bval \<Rightarrow> bool" ("_ \<turnstile> _ \<Down> _" [50,50,50] 51) where
  ...
  eval_cons[intro!]: "\<lbrakk> \<rho> \<turnstile> e1 \<Down> v1; \<rho> \<turnstile> e2 \<Down> v2 \<rbrakk> \<Longrightarrow> \<rho> \<turnstile> ECons e1 e2 \<Down> BPair v1 v2" |
  eval_car[intro!]: "\<lbrakk> \<rho> \<turnstile> e \<Down> BPair v1 v2 \<rbrakk> \<Longrightarrow> \<rho> \<turnstile> ECar e \<Down> v1" |
  eval_cdr[intro!]: "\<lbrakk> \<rho> \<turnstile> e \<Down> BPair v1 v2 \<rbrakk> \<Longrightarrow> \<rho> \<turnstile> ECdr e \<Down> v2"
...
inductive_cases 
  ...
  eval_if_inv[elim!]: "\<rho> \<turnstile> EIf e1 e2 e3 \<Down> v" and
  eval_cons_inv[elim!]: "\<rho> \<turnstile> ECons e1 e2 \<Down> v" and
  eval_car_inv[elim!]: "\<rho> \<turnstile> ECar e \<Down> v" and
  eval_cdr_inv[elim!]: "\<rho> \<turnstile> ECdr e \<Down> v"
...
fun psubst :: "env \<Rightarrow> exp \<Rightarrow> exp" where
  ...
  "psubst \<rho> (ECons e1 e2) = ECons (psubst \<rho> e1) (psubst \<rho> e2)" |
  "psubst \<rho> (ECar e) = ECar (psubst \<rho> e)" |
  "psubst \<rho> (ECdr e) = ECdr (psubst \<rho> e)"
...
inductive bs_val :: "bval \<Rightarrow> exp \<Rightarrow> bool" and
  ...
  bs_pair[intro!]: "\<lbrakk> bs_val w1 v1 \<and> bs_val w2 v2 \<and> is_val v1 \<and> is_val v2 \<rbrakk> \<Longrightarrow>
                    bs_val (BPair w1 w2) (ECons v1 v2)" |
  bs_nil[intro!]: "bs_env [] []" |
  bs_cons[intro!]: "\<lbrakk> bs_val w v; bs_env \<rho> \<rho>' \<rbrakk> \<Longrightarrow> bs_env ((x,w)#\<rho>) ((x,v)#\<rho>')" 

text{*bs_pair[intro!]: "\<lbrakk> bs_val w1 v1; bs_val w2 v2 \<rbrakk> \<Longrightarrow> bs_val (BPair w1 w2) (ECons v1 v2)" |*}

inductive_cases bs_env_inv1[elim!]: "bs_env ((x, w) # \<rho>) \<rho>'" and
  bs_clos_inv[elim!]: "bs_val (BClos x e \<rho>'') v1" and
  bs_nat_inv[elim!]: "bs_val (BNat n) v" and
  bs_pair_inv[elim!]: "bs_val (BPair v1 v2) v"
...
lemma cons_red_cong1: " e1 \<longrightarrow>* e1' \<Longrightarrow> ECons e1 e2 \<longrightarrow>* ECons e1' e2"
  apply (case_tac "\<not> is_val e1")
   apply (induction rule: multi_step.induct) apply blast apply simp 
   apply (case_tac "is_val e2a") apply simp
    apply (metis cons_left ms_cons ms_nil multi_step.cases stuck_def val_stuck)
  apply simp apply (meson cons_left ms_cons stuck_def val_stuck)
  by (metis is_val_def multi_step.simps stuck_def val_stuck)

lemma cons_red_cong2: "e2 \<longrightarrow>* e2' \<Longrightarrow> ECons e1 e2 \<longrightarrow>* ECons e1 e2'"
  apply (case_tac "\<not> is_val e2")
   apply (induction rule: multi_step.induct) apply blast apply simp 
   apply (case_tac "is_val e1a") apply simp
  apply (metis cons_right ms_cons ms_nil multi_step.cases stuck_def val_stuck)
  apply simp by (metis ms_nil multi_step.cases stuck_def val_stuck)

lemma car_red_cong1: "e \<longrightarrow>* e' \<Longrightarrow> ECar e \<longrightarrow>* ECar e'" 
  by (induction rule: multi_step.induct) blast+

lemma cdr_red_cong1: "e \<longrightarrow>* e' \<Longrightarrow> ECdr e \<longrightarrow>* ECdr e'" 
  by (induction rule: multi_step.induct) blast+
...
next
  case (eval_cons \<rho> e1 v1 e2 v2 \<rho>')
  from eval_cons have "FV e1 \<subseteq> set (map fst \<rho>)" by auto
  from this eval_cons obtain v1' where e1_v1: "psubst \<rho>' e1 \<longrightarrow>* v1'" and 
    v1_v1: "bs_val v1 v1'" by blast
  from eval_cons have "FV e2 \<subseteq> set (map fst \<rho>)" by auto
  from this eval_cons obtain v2' where e2_v2: "psubst \<rho>' e2 \<longrightarrow>* v2'" and 
    v2_v2: "bs_val v2 v2'" by blast

  from v1_v1 v2_v2 have val_cons: "bs_val (BPair v1 v2) (ECons v1' v2')" by auto

  from e1_v1 have 1: "ECons (psubst \<rho>' e1) (psubst \<rho>' e2) \<longrightarrow>* ECons v1' (psubst \<rho>' e2)" 
    by (rule cons_red_cong1)
  from e2_v2 have 2: "ECons v1' (psubst \<rho>' e2) \<longrightarrow>* ECons v1' v2'" 
    by (rule cons_red_cong2)

  from 1 2 have 3: "psubst \<rho>' (ECons e1 e2) \<longrightarrow>* ECons v1' v2'" apply simp 
    apply (rule multi_step_trans) apply auto done
  from this val_cons show ?case apply (rule_tac x="ECons v1' v2'" in exI) by blast
next
  case (eval_car \<rho> e v1 v2 \<rho>')
  from eval_car have "FV e \<subseteq> set (map fst \<rho>)" by auto
  from this eval_car obtain pv where e_pv: "psubst \<rho>' e \<longrightarrow>* pv" and
    val_pv: "is_val pv" and pair_pv: "bs_val (BPair v1 v2) pv" by blast
  from pair_pv obtain v1' v2' where pv_econs:"pv = ECons v1' v2'" and 
    v1_v1: "bs_val v1 v1'" and v2_v2:"bs_val v2 v2'" by auto

  from e_pv pv_econs have 0: "psubst \<rho>' e \<longrightarrow>* (ECons v1' v2')" by auto
  from this have 2: "ECar (psubst \<rho>' e) \<longrightarrow>* ECar (ECons v1' v2')" by  (rule car_red_cong1)

  from val_pv pv_econs have val_v1: "is_val v1'" and val_v2: "is_val v2'" by auto
  from val_v1 val_v2 have 3: "ECar (ECons v1' v2') \<longrightarrow>* v1'" by blast

  have 1:  "psubst \<rho>' (ECar e) \<longrightarrow>* ECar (psubst \<rho>' e)" by auto
  from 1 2 have step: "psubst \<rho>' (ECar e) \<longrightarrow>* ECar (ECons v1' v2')" by auto
  from this 3 have final: "psubst \<rho>' (ECar e) \<longrightarrow>* v1'" by (rule multi_step_trans)

  from val_v1 v1_v1 final show ?case apply (rule_tac x="v1'" in exI) by blast
next
  case (eval_cdr \<rho> e v1 v2 \<rho>')
  from eval_cdr have "FV e \<subseteq> set (map fst \<rho>)" by auto
  from this eval_cdr obtain pv where e_pv: "psubst \<rho>' e \<longrightarrow>* pv" and
    val_pv: "is_val pv" and pair_pv: "bs_val (BPair v1 v2) pv" by blast
  from pair_pv obtain v1' v2' where pv_econs:"pv = ECons v1' v2'" and
    v1_v1:" bs_val v1 v1'" and v2_v2: "bs_val v2 v2'" by auto

  from e_pv pv_econs have 0: "psubst \<rho>' e \<longrightarrow>* (ECons v1' v2')" by auto
  from this have 2: "ECdr (psubst \<rho>' e) \<longrightarrow>* ECdr (ECons v1' v2')" by (rule cdr_red_cong1)

  from val_pv pv_econs have val_v1: "is_val v1'" and val_v2: "is_val v2'" by auto
  from val_v1 val_v2 have 3: "ECdr (ECons v1' v2') \<longrightarrow>* v2'" by blast

  have 1: "psubst \<rho>' (ECdr e) \<longrightarrow>* ECdr (psubst \<rho>' e)" by auto
  from 1 2 have step: "psubst \<rho>' (ECdr e) \<longrightarrow>* ECdr (ECons v1' v2')" by auto
  from this 3 have final: "psubst \<rho>' (ECdr e) \<longrightarrow>* v2'" by (rule multi_step_trans)

  from val_v2 v2_v2 final show ?case apply (rule_tac x="v2'" in exI) by blast
next
...
fun bs_observe :: "bval \<Rightarrow> obs \<Rightarrow> bool" where
  ...
  "bs_observe (BPair bval1 bval2) OPair = True" |
...
proof -
  ...
  from this 3 4 5 show ?thesis apply (rule_tac x=v' in exI) apply simp
    apply (case_tac v)
     ...
     apply (rule_tac x=OFun in exI) apply force
     apply (rule_tac x=OPair in exI) apply force done
qed
