...
fun E :: "exp \<Rightarrow> env \<Rightarrow> val set" where
  ...
  Econs: "E (ECons e1 e2) \<rho> = {p. \<exists> v1 v2. v1 \<in> E e1 \<rho> \<and> v2 \<in> E e2 \<rho> \<and>  p = VPair v1 v2}" |
  Ecar: "E (ECar e) \<rho> = {v1. \<exists> v2. (VPair v1 v2) \<in> E e \<rho>}" |
  Ecdr: "E (ECdr e) \<rho> = {v2. \<exists> v1. (VPair v1 v2) \<in> E e \<rho>}"
